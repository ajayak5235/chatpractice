...
function addMessage(data){
    console.log('my input send>>>>>',data)
    if(!data){
        return;
    }
    chats.innerHTML= '';
    ...
    if(token){
        ...
        if(groupID){
            ...
            async function getMessages() {
               // console.log()
                try {
                    // Assuming groupID is a variable representing the current group's ID
                    const response = await axios.get(`/user/getMessages/${groupID}`, { headers: { "Authorization": token } });
            ...
            // Update the form ID to match the one in your HTML
const form = document.getElementById('messageForm');
